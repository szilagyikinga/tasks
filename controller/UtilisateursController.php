...
class UtilisateursController extends Controller
{
        ...
        if ($this->request->data) {
            ...
            if ($this->Utilisateur->validates($data)) {
                ...
                if (!empty($user)) {
                    ...
                    $this->Session->setFlash('Ce compte n\'existe pas', 'error');
                }
            } else {
    ...
    /**
     * Vérifie si l'utilisateur a des nouvelles tâches attribuées
     **/
    private function pendingUserTask()
    {
        $this->loadModel('Tache');
        $nbTache = $this->Tache->findCount(array(
            'responsable' => $this->request->data->login,
            'en_ligne' => 1,
            'attribution_acceptee' => 0
        ));
        if (!empty($nbTache)) return $nbTache;
        return false;
