...
class Form
{
    ...
    public function input($name, $label, $options = array(), $emptyDefaultValue = "Non sélectioné")
    {
        $error = false;
        $classError = '';
        ...
        if (!isset($options['type']) && !isset($options['options'])) {
            ...
            foreach ($options['options'] as $v) {
                if ($v === "") $html .= '<option value="' . $v . '" ' . ($v == $value ? 'selected' : '') . '>' . $emptyDefaultValue . '</option>';
                else $html .= '<option value="' . $v . '" ' . ($v == $value ? 'selected' : '') . '>' . $v . '</option>';
            }
            $html .= '</select>';
